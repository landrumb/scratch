...
fn bench_graph_read(c: &mut Criterion) {
    ...
    for (name, size, max_degree, avg_degree) in configs {
        ...
        graph.save(&graph_path).expect("Failed to save benchmark graph");
    ...
    for (name, size, max_degree, avg_degree) in configs {
        ...
        group.bench_with_input(BenchmarkId::new("write", name), &graph, |b, g| {
            b.iter(|| {
                ...
                g.save(&graph_path).expect("Failed to write graph in benchmark");
            });
        });
    }
...
fn bench_graph_roundtrip(c: &mut Criterion) {
    ...
    for (name, size, max_degree, avg_degree) in configs {
        ...
        let graph = create_random_graph(size as IndexT, max_degree, avg_degree);
        graph.save(&input_path).expect("Failed to save initial benchmark graph");
        
        // Benchmark the round-trip
        group.bench_with_input(BenchmarkId::new("roundtrip", name), &(input_path, output_path), |b, (in_path, out_path)| {
            b.iter(|| {
                let graph = ClassicGraph::read(in_path).expect("Failed to read graph in roundtrip benchmark");
                graph.save(out_path).expect("Failed to write graph in roundtrip benchmark");
            });
        });
    }
...
fn bench_graph_creation(c: &mut Criterion) {
    ...
    for (name, size, max_degree, avg_degree) in configs {
        group.bench_with_input(BenchmarkId::new("create", name), &(size, max_degree, avg_degree), |b, &(size, max_degree, avg_degree)| {
            b.iter(|| {
                create_random_graph(size as IndexT, max_degree, avg_degree)
            });
        });
    }
