...
def plot_clique_histogram(graph_file: Path, output_dir: Path) -> None:
    ...
    print(
        f"\tpoints covered: {maximal_clique_coverage:,} ({maximal_clique_coverage / graph.n * 100:.2f}%)"
    )
    ...
    for c in tqdm(cliques, desc="validating that the maximal cliques actually exist"):
        for i in range(len(c)):
            ...
            for other in others:
                assert other in neighbors, (
                    f"edge {point}-{other} is not in clique {c} (point {point} has neighbors {neighbors}, {other} has neighbors {graph.get_neighborhood(other)})"
                )
    print("done")
    ...
    plt.savefig(
        output_dir / "maximal_clique_histogram.png", bbox_inches="tight", dpi=300
    )
    plt.close()
    ...
    print(
        f"\tpoints covered: {independent_clique_coverage:,} ({independent_clique_coverage / graph.n * 100:.2f}%)"
    )

    for c in tqdm(
        independent_cliques,
        desc="validating that the maximal independent cliques actually exist",
    ):
        for i in range(len(c)):
            point = c[i]
            others = c[:i] + c[i + 1 :]
    ...
    plt.xticks(range(1, max(sizes) + 1))
    plt.savefig(
        output_dir / "maximal_independent_clique_histogram.png",
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
    ...
    print(
        f"points covered by independent cliques: {sum(sizes):,} ({sum(sizes) / graph.n * 100:.2f}%)"
    )
    print(
        f"points not covered by independent cliques: {graph.n - sum(sizes):,} ({100 - sum(sizes) / graph.n * 100:.2f}%)"
    )
    print(
        f"total nodes in a graph where independent cliques are contracted: {graph.n - sum(sizes) + len(independent_cliques):,} ({100 - (graph.n - sum(sizes) + len(independent_cliques)) / graph.n * 100:.2f}% reduction)"
    )
    ...
    plt.xticks(range(1, max(sizes) + 1))
    plt.savefig(
        output_dir / "independent_clique_size_weighted_histogram.png",
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
