...
for i in range(graph.n):
    ...
    for j in neighbors:
        plt.plot(
            [subset_points[i][0], subset_points[j][0]],
...
for i, neighbor_idx in enumerate(neighbors):
    ...
    plt.plot(
        [center[0], neighbor[0]], [center[1], neighbor[1]], color=color, linewidth=2.0
    )
    ...
    plt.scatter(
        ...
        edgecolor="black",
        linewidth=1.0,
        label=f"Neighbor {i + 1} ({neighbor_idx})",
    )
    ...
    dist_to_center_grid = np.sqrt(np.sum((grid_points - center) ** 2, axis=1)).reshape(
        X.shape
    )
    dist_to_neighbor_grid = np.sqrt(
        np.sum((grid_points - neighbor) ** 2, axis=1)
    ).reshape(X.shape)
    ...
    contour = plt.contour(
        X, Y, Z, levels=[0], colors=[color], linestyles="dashed", linewidths=1.5
    )
...
def greedy_search(graph, dataset, query_point, start_idx=None, max_steps=20):
    ...
    for step in range(max_steps):
        ...
        current_distance = euclidean_distance(
            dataset.get_vector(current_idx), query_point
        )
...
distance_ratio = (
    greedy_distance / exact_nn_distance if exact_nn_distance > 0 else float("inf")
)
...
plt.plot(
    [query_point[0], exact_nn_point[0]],
    [query_point[1], exact_nn_point[1]],
    "magenta",
    linestyle="-",
    linewidth=1.5,
)
...
for i, (x, y) in enumerate(path_points):
    plt.text(
        x + 0.01, y + 0.01, str(i), fontsize=10, bbox=dict(facecolor="white", alpha=0.7)
    )
...
print(
    f"Query point (bottom left): [{query_point_animated[0]:.2f}, {query_point_animated[1]:.2f}]"
)

# Find the exact nearest neighbor for the animated search
exact_nn_idx_animated, exact_nn_distance_animated = find_exact_nearest_neighbor(
    dataset, query_point_animated
)
exact_nn_point_animated = dataset.get_vector(exact_nn_idx_animated)
print(
    f"Exact nearest neighbor: index {exact_nn_idx_animated}, distance {exact_nn_distance_animated:.4f}"
)
...
# Calculate the ratio of greedy search distance to exact NN distance
distance_ratio_animated = (
    greedy_distance_animated / exact_nn_distance_animated
...
def update_frame(frame_num):
    ...
    ax.scatter(
        query_point_animated[0],
        query_point_animated[1],
        color="red",
        s=200,
        marker="*",
        edgecolor="k",
        linewidth=2.0,
        label="Query Point (Target)",
        zorder=10,
    )

    # Draw the exact nearest neighbor (optimal solution)
    ax.scatter(
        exact_nn_point_animated[0],
        exact_nn_point_animated[1],
        color="magenta",
        s=140,
        marker="X",
        edgecolor="k",
    ...
    ax.plot(
        [query_point_animated[0], exact_nn_point_animated[0]],
    ...
    if not final_frame and frame_num > 0:
        ...
        for neighbor_idx in next_step["explored_neighbors"]:
            ...
            ax.plot(
                [current_pos[0], neighbor_pos[0]],
                [current_pos[1], neighbor_pos[1]],
                "gray",
                linestyle=":",
                linewidth=1.0,
                alpha=0.6,
            ...
            if neighbor_idx in next_step["distances"]:
                ...
                max_dist = (
                    max(next_step["distances"].values())
                    if next_step["distances"]
                    else 1.0
                )
                min_dist = (
                    min(next_step["distances"].values())
                    if next_step["distances"]
                    else 0.0
                )
                dist_range = max_dist - min_dist if max_dist != min_dist else 1.0
                normalized_dist = (
                    max_dist - next_step["distances"][neighbor_idx]
                ) / dist_range
                size = (
                    40 + normalized_dist * 60
                )  # Size between 40 and 100 based on distance
...
ani = animation.FuncAnimation(
    fig, update_frame, frames=num_frames, interval=800, blit=True, repeat=True
)
