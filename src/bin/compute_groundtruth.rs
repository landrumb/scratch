...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let neighbors_count = matches
        .get_one::<String>("neighbors")
        .unwrap()
        .parse::<usize>()?;
    ...
    println!(
        "Dataset loaded in {:?}: {} points with {} dimensions",
        start.elapsed(),
        dataset.size(),
        dataset.dim
    );
    ...
    let query_dataset = if let Some(query_path) = matches.get_one::<String>("query_set") {
        ...
        println!(
            "Query set loaded in {:?}: {} points with {} dimensions",
            q_start.elapsed(),
            queries.size(),
            queries.dim
        );
    ...
    println!("Computing ground truth for {neighbors_count} neighbors");
    ...
    println!(
        "Ground truth computation completed in {:?}",
