...
fn main() {
    ...
    println!("Average degree: {:.2}", total_edges as f64 / graph.size() as f64);
    ...
    println!("Graph written in {:.3} seconds", write_elapsed.as_secs_f64());
    ...
    if args.len() > 3 && args[3] == "--verify" {
        ...
        if verified_graph.n != graph.n {
            println!("❌ Node count mismatch: {} vs {}", graph.n, verified_graph.n);
            is_equal = false;
        }
        
        if verified_graph.r != graph.r {
            println!("❌ Max degree mismatch: {} vs {}", graph.r, verified_graph.r);
            is_equal = false;
        }
        ...
        for i in 0..graph.n {
            ...
            if orig_neighbors.len() != new_neighbors.len() {
                println!("❌ Degree mismatch for node {}: {} vs {}", 
                          i, orig_neighbors.len(), new_neighbors.len());
                edge_mismatch = true;
                is_equal = false;
                break;
            }
        ...
        if is_equal {
            println!("✅ Verification successful in {:.3} seconds", verify_elapsed.as_secs_f64());
