...
mod tests {
    ...
    fn test_simple_graph() {
        ...
        for i in 0..graph.n {
            ...
            assert_eq!(original_neighbors.len(), loaded_neighbors.len(), 
                       "Node {} has different number of neighbors", i);
            
            for j in 0..original_neighbors.len() {
                assert_eq!(original_neighbors[j], loaded_neighbors[j], 
                           "Neighbors don't match for node {} at position {}", i, j);
            }
        }
    ...
    fn test_varying_degrees() {
        ...
        for i in 0..graph.n {
            ...
            assert_eq!(loaded_graph.degree(i), expected_degree, 
                       "Node {} has wrong degree", i);
    ...
    fn test_max_capacity() {
        ...
        for i in 0..graph.n {
            assert_eq!(loaded_graph.degree(i), r, 
                       "Node {} doesn't have max degree", i);
    ...
    fn test_append_neighbors() {
        ...
        // Load
        let loaded_graph = ClassicGraph::read(&path).expect("Failed to read graph with appended neighbors");
    ...
    fn test_read_invalid_header() {
        ...
        assert!(result.is_err(), "Reading file with invalid header should fail");
