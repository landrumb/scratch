...
use crate::{clustering::kmeans::kmeans, data_handling::{dataset::VectorDataset, dataset_traits::Dataset}, distance::euclidean::euclidean, graph::graph::IndexT};
...
pub fn brute_force(query: &[f32], points: &[f32]) -> Vec<(IndexT, f32)> {
    ...
    let mut results: Vec<(IndexT, f32)> = (0..n).map(|i| (i as IndexT, euclidean(&query, &points[i*dim..(i+1)*dim], dim).to_f32().unwrap())).collect();
...
impl<'a> IVFIndex<'a> {
    ...
    pub fn new(dataset: &'a VectorDataset<f32>, representatives: Vec<f32>, partitions: Vec<Vec<IndexT>>) -> IVFIndex<'a> {
        assert!(representatives.len() == partitions.len(), "Number of representatives ({}) mismatched with number of partitions ({})", representatives.len(), partitions.len());
    ...
    pub fn build(dataset: &'a VectorDataset<f32>, k: usize, max_iter: usize, epsilon: f32) -> IVFIndex<'a> {
        let (representatives, assignments) = kmeans(dataset, k, max_iter, epsilon.to_f64().unwrap());
        ...
        IVFIndex {k, dataset, representatives, partitions}
    }

    /// get a slice with a given representative
    pub fn get_representative(&self, idx: usize) -> &[f32] {
        assert!(idx < self.k, "Representative index out of bounds: {} >= {}", idx, self.k);
    ...
    pub fn query_partition(&self, query: &[f32], partition: usize) -> Vec<(IndexT, f32)> {
        assert!(partition < self.k, "Partition index out of bounds: {} >= {}", partition, self.k);

        let mut results = (0..self.partitions[partition].len())
            .into_par_iter()
            .map(|i| {
                let idx = self.partitions[partition][i];
                (idx, self.dataset.compare(query, idx as usize).to_f32().unwrap())
            })
    ...
    pub fn query_representatives(&self, query: &[f32]) -> Vec<(IndexT, f32)> {
        let mut results = (0..self.k)
            ...
            .map(|i| {
                let idx = i as IndexT;
                (idx, euclidean(query, self.get_representative(i), self.dataset.dim).to_f32().unwrap())
            })
            .collect::<Vec<(IndexT, f32)>>();
